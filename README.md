...
- Vehicle cards use images copied from the website's own asset library (`assets/vehicles/*.jpg`). **This line used to claim they were "real LCT Universal fleet photography — not stock images". That was wrong, and it is corrected here rather than quietly deleted.** They are byte-identical (md5, checked 2026-09-02) to the site's `public/assets/fleet-{sedan,suv,sprinter}.jpg` — which are studio *renders*, and which **the website itself has superseded**: its `src/lib/image-map.ts` now points all three fleet classes at client-supplied photographs under `public/assets/official/`. The app is a version behind, and two of the three renders are wrong about the product (an S-Class on gold aftermarket wheels; a *cargo* Sprinter illustrating a 14-passenger class). See `OPEN_QUESTIONS.md` #13 — moving to the real photographs is **blocked on a business question, not on effort.** Onboarding slides come from the same library and have not been re-checked.
