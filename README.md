...
- Vehicle cards: **Executive Sedan and Sprinter use the client's own photographs**, copied in and resized from the website's `public/assets/official/`. The **Executive SUV still uses a studio render**, deliberately — see `OPEN_QUESTIONS.md` #13.

  This line used to claim all of them were "real LCT Universal fleet photography — not stock images". **That was wrong, and it is corrected here rather than quietly deleted.** Until 2026-09-02 the files were byte-identical (md5) to the site's superseded `fleet-{sedan,suv,sprinter}.jpg` *renders* — two of which were wrong about the product: an S-Class on gold aftermarket wheels, and a *cargo* Sprinter illustrating a 14-passenger class. The real photography existed the whole time, one directory away. Onboarding slides come from the same library and **have not been re-checked**.
